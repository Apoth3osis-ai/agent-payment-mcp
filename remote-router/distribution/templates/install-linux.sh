#!/bin/bash
# AgentPMT MCP Router Installer for Linux
...
echo "AgentPMT MCP Router Installer for Linux"
echo "========================================"
echo ""
...
if [ "$CLIENT" = "claude" ]; then
    INSTALL_PATH="$HOME/.config/Claude/servers/agent-payment"
...
ARCH=$(uname -m)
if [ "$ARCH" = "x86_64" ]; then
    BINARY_URL="https://github.com/Apoth3osis-ai/agent-payment-mcp/releases/latest/download/agent-payment-router-linux-amd64"
elif [ "$ARCH" = "aarch64" ] || [ "$ARCH" = "arm64" ]; then
    BINARY_URL="https://github.com/Apoth3osis-ai/agent-payment-mcp/releases/latest/download/agent-payment-router-linux-arm64"
else
...
if [ -f "$CONFIG_PATH" ]; then
    ...
    if command -v jq > /dev/null; then
        ...
        echo "   Please manually add agent-payment to $CONFIG_PATH"
    fi
else
    # Create new config
